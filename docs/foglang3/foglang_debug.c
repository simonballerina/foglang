...
void print_ast_statement(Node* node, const char* prefix, int is_left) {
    ...
    switch (node->type) {
        ...
        case NODE_BINARY: {
            ...
            print_ast_statement(node->binary.left, new_prefix, 1);

            print_ast_statement(node->binary.right, new_prefix, 0);
        ...
        case NODE_BAND: {
            ...
            print_ast_statement(node->band.value, new_prefix, 0);
        ...
        case NODE_FOUG: {
            ...
            print_ast_statement(node->foug.string, new_prefix, 0);
        ...
        case NODE_TPOS: {
            ...
            print_ast_statement(node->tpos.string, new_prefix, 0);
        ...
        case NODE_GIVET: {
            ...
            */
            ...
            for (int i = 0; i < node->block.statement_count; i++) {
                print_ast_statement(node->block.block[i], block_prefix,
                                    i != node->block.statement_count - 1);
            }
    ...
    for (int i = 0; i < instruction_amount; i++) {
        switch (instructions[i].type) {
            ...
            case OP_MOD:
                ...
                break;
            case IDENTIFIER:
                printf("'%.*s'    ", (int)strlen(instructions[i].string),
