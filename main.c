...
void band2(char* variables_names, double* variables_values, int* global_var_index, char* current_instruction, int debug, int* program_counter){
    if (debug == 2) printf("%d      BAND\n", *program_counter);
    // hitta argumenten till band 
    char args[128];
    int instr_len = strlen(current_instruction);
    for (int i = 5; i < instr_len+1; i++){
        args[i-5] = current_instruction[i];
